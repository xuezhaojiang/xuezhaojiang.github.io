...
#### 1. 使用Huffman编码  
正常的Huffman算法的关键在于统计出各符号出现的频率，以及正确建立哈夫曼树。算法比较简单。需要两次扫描，而且解压缩需要编码表。  
本次处理的天文学照片，背景颜色占了绝大多数，频率在高的字符几乎占到了总字符的50%，这就意味着，剩下的字符出现的频率相差无几，导致建树编码复杂，并且建立哈夫曼树硬件上实现比较困难，这样建立哈夫曼树的效果就显得没有必要。  
...
#### 2.UDP传输文件  
tcp是在运输层来检查是否有丢失数据，udp并不检查这一点，为了做的不丢包，只能在应用层检查，也就是接收方给予反馈。  
...
![效果图](file/SoftwareEngineeringPractice/speed.jpg)    
  
主要是前端每10s发送一个请求，请求中包含一个区域，后台反馈给前端这个区域车辆的平均车速，前端用动态折线图显示出来。来判断是否堵车。虽然我认为用这个方法不能判断是否拥堵。看起来很厉害的样子，但前端真的是痛苦，坚决不写前端。  
